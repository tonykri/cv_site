    ...
    return(
        <div className="container mx-auto my-5">
            {viewSuccess && <div id="toast-success" className=" m-auto mt-5 flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800" role="alert">
                ...
                <div className="ml-3 text-sm font-normal">Deleted successfully.</div>
            ...
            {data.length === 0 && <div className="w-full h-full flex justify-center">
                <h1 className="text-2xl text-center mt-4">No requests</h1>
            </div>
            }
            {data.map(notification => (<NotificationCard key={notification.applicationId} notification={notification} Refresh={Refresh} setViewSuccess={setViewSuccess} />))}
        </div>
    )
}